...
def detect_category(query):
    ...
    if any(word in query.lower() for word in ["show all", "list all", "overview", "details"]):
        return "General"
...
def search_info(query, category):
    ...
    general_queries = {
        "show all project details": df_project,
        "list all projects": df_project,
        "give me an overview of all scrum masters": df_scrum,
        "list all developers and their roles": df_dev,
        "show all leads and their projects": df_lead
    }

    for q, df in general_queries.items():
        if q in query.lower():
...
if st.button("Search 💬"):
    if user_input.strip():
        ...
        if category:
            ...
            if isinstance(result, list):  # Handle multiple records for general queries
                ai_response = f"✅ {category} Details:<br><br>"
                for item in result[:5]:  # Show first 5 results for readability
                    ai_response += "<br>".join([f"**{k}:** {v}" for k, v in item.items()])
                    ai_response += "<br>-------------------<br>"
            elif isinstance(result, dict):  # Single record found
